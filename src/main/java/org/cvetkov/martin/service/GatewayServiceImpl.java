...
public class GatewayServiceImpl implements GatewayService{
	...
	private String deviceExists(Device device) {
		Device existingDevice = deviceJpaRepository.findByUid(device.getUid());
		if (existingDevice != null) {
	...
	@Override
	public void updateGateway(String serialNumber, GatewayUpdate gatewayUpdate) {
		if (gatewayJpaRepository.findBySerialNumber(serialNumber) == null) {
			throw new GatewayNotFoundException(serialNumber);
		}
		
		if(StringUtils.isEmpty(gatewayUpdate.getName())){
			throw new InvalidRequestException();
		}
		
		if(!Validator.validIp4Address(gatewayUpdate.getIpAddress())) {
			throw new IpAddressNotValidException(serialNumber, gatewayUpdate.getIpAddress());
		}
		
		gatewayJpaRepository.updateGateway(serialNumber, gatewayUpdate.getName(), gatewayUpdate.getIpAddress());
	}

	@Override
	public void updateDevice(String serialNumber, int deviceId, DeviceUpdate deviceUpdate) {
		if (deviceJpaRepository.findByGatewaySerialNumberAndUid(serialNumber, deviceId) == null) {
			throw new DeviceNotFoundException(serialNumber, deviceId);
		}
		
		if(StringUtils.isEmpty(deviceUpdate.getVendor())){
			throw new InvalidRequestException();
		}
		
		deviceJpaRepository.updateDevice(serialNumber, deviceId,
				                           deviceUpdate.getVendor(),
				                           deviceUpdate.getDateCreated(),
