...
import org.cvetkov.martin.utilities.DeviceResponse;
...
public class DevicesController {
	...
	@RequestMapping(value = "/gateways/{gatewaySerialNumber}/devices/{deviceUID}", method = RequestMethod.PUT)
	@ResponseStatus(HttpStatus.OK)
	public DeviceResponse updateDevice(@PathVariable("gatewaySerialNumber") String serialNumber,
			                           @PathVariable("deviceUID") int deviceId,
			                           @RequestBody DeviceUpdate deviceUpdate) {

		service.updateDevice(serialNumber, deviceId, deviceUpdate);
		return new DeviceResponse("Device with id " + deviceId + " was updated.");
	}
